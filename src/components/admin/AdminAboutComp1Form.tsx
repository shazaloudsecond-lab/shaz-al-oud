...
export default function AdminAboutComp1Form() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching about section 1:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDeleteImage = async () => {
    ...
    if (configId) {
      try {
        ...
        console.error("Error deleting about image in DB:", e);
      }
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (configId) {
        ...
        setStatusMsg({ type: "success", text: "Component 1 updated successfully!" });
      } else {
        const { data, error } = await supabase
          .from("about_section")
          .insert([payload])
        ...
        setStatusMsg({ type: "success", text: "Component 1 saved successfully!" });
      }
    } catch (err: any) {
      setStatusMsg({ type: "error", text: err.message || "Failed to save Component 1." });
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="bg-white border border-neutral-200 rounded-2xl p-6 sm:p-8 space-y-6 shadow-sm">
          ...
          <div className="flex items-center justify-between border-b border-neutral-200 pb-4">
            <div>
              ...
              <p className="text-xs text-neutral-500 mt-0.5">
                Layout: Left side Image, Right side Content
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              {imageUrl ? (
                <div className="relative w-full sm:w-48 aspect-[4/3] rounded-xl overflow-hidden bg-neutral-100 border border-neutral-200 flex-shrink-0">
                  <img src={imageUrl} alt="About 1 Preview" className="w-full h-full object-cover" />
                  <button
                    type="button"
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. The Art of Pure Luxury & Craftsmanship"
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. A Legacy of Authentic Arabian Perfumery"
              className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-300 rounded-lg text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-black text-sm font-medium"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Write the introduction and background..."
          ...
          <div className="space-y-2">
            <label className="block text-xs uppercase tracking-wider text-neutral-700 font-bold font-mono">
              Brand Story / Secondary Paragraph
            </label>
            <textarea
              rows={4}
              value={story}
              onChange={(e) => setStory(e.target.value)}
              placeholder="Write details about pure oud sourcing and heritage..."
              className="w-full px-4 py-2.5 bg-neutral-50 border border-neutral-300 rounded-lg text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-black text-sm font-medium resize-none leading-relaxed"
            />
          </div>
          ...
          <div className="flex justify-end pt-4 border-t border-neutral-200">
            ...
            >
              {saving ? "Saving Changes..." : "Save Component 1"}
            </button>
          </div>
        </div>
      </form>
